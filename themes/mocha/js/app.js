/**
 * Mocha — Catppuccin Mocha 主题
 */
;(function () {
  'use strict'

  const ACCENT_PRESETS = {
    sapphire:  { primary: '#74c7ec', light: '#89dceb', accent: '#cba6f7' },
    mauve:     { primary: '#cba6f7', light: '#dbb9fd', accent: '#74c7ec' },
    sky:       { primary: '#89dceb', light: '#a1e8f5', accent: '#cba6f7' },
    teal:      { primary: '#94e2d5', light: '#b0ebe0', accent: '#89b4fa' },
    peach:     { primary: '#fab387', light: '#fcc5a3', accent: '#cba6f7' },
    rosewater: { primary: '#f5e0dc', light: '#f8edea', accent: '#74c7ec' },
  }
  ...
  function injectRingGradient() {
    ...
    const c1 = cs.getPropertyValue('--primary').trim() || '#74c7ec'
    const c2 = cs.getPropertyValue('--accent').trim() || '#cba6f7'
  ...
  function applyThemeSettings(ts) {
    ...
    const accentKey = ts.accent_color || 'sapphire'
    const preset = ACCENT_PRESETS[accentKey] || ACCENT_PRESETS.sapphire
  ...
  async function loadChart() {
    ...
    try {
      ...
      const cs = getComputedStyle(document.documentElement)
      const primary = cs.getPropertyValue('--primary').trim() || '#74c7ec'
      const accent = cs.getPropertyValue('--accent').trim() || '#cba6f7'
      const muted = cs.getPropertyValue('--overlay-0').trim() || '#6c7086'
        ...
        series: [
          {
            ...
            itemStyle: { color: accent, borderColor: '#1e1e2e', borderWidth: 2 },
          },
        ],
      })
